var activity = {
	...
	setThings:function() {
		...
		this.view.appendChild(actionBar.view);

		this.iconContainer = new Label();
		this.iconContainer.setPadding(14, 0, 14, 0);
		actionBar.addView(this.iconContainer);
		this.iconContainer.view.style.display = 'none';
		...
		this.view.appendChild(body.view);
	...
	setDisplayAsUp:	function(b) {
		if (b) {
			this.iconContainer.view.style.display = 'inline-block';
		}
		else{
			this.iconContainer.view.style.display = 'none';
		}
	...
	inflateMenu:function (container, resource) {
		var new_str = '';
	...
	inflateDrawer: function (resource) {
		$.get(resource, function(response, status) {
			try{
				var first = null;
